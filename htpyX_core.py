...
def w(value, name=None):
    """Create a W-Var (Weird Technologia variable)"""
    var = WVar(value, name)
    if name:
        _wvars[name] = var
    return var
    ...
    def creator(*kids, **attrs):
        ...
        for kid in kids:
            if isinstance(kid, WVar):
                processed_kids.append(kid.name)
            else:
...
def export_html(root, filename="index.html", extra_js=""):
    """Export to HTML with W-Tech declarations"""
    try:
...
{extra_js}
        ...
        debug(f"✅ Exported HTML to {filename}")
        clear_wvars()
